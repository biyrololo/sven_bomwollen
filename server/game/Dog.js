...
export default class Dog extends Entity {
    constructor(position) {
        ...
        // console.log(`DOG this max move time ${this.max_move_time}`);
    }

    move_to_sheep(){
        let sheep = this.game.get_next_closes_sheep(this, `dog`);
        // console.log(`found SHEEP`, sheep.id, sheep.position);
        // console.log(`start move to sheep ${sheep.id}`);
        // console.log('sheep', sheep);
        if(sheep){
            // console.log('try move to sheep', sheep.id);
            sheep.checked_by_dog = true;
            let path = find_shortest_path(this.position, sheep.position, this.game.map);
            // console.log('path to sheep', path);
            // console.log(path)
            if(path.length > 0){
                // console.log('start move to sheep', sheep.id);
